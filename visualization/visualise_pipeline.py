...
3 rows × 3 cols:
    Original + GT boxes  |  Raw Attention  |  Grad-CAM
Each heatmap panel has 4 per-image metric chips annotated at the bottom.
Chip colour: green = this method wins on this metric, red = loses.
...
def _annotate_metrics(ax, m_this, m_other):
    """4 metric chips at the bottom of ax. Green = wins, red = loses."""
    chips = [
        ("PGA",  "✓" if m_this["pga"] else "✗", m_this["pga"] > m_other["pga"] or (m_this["pga"] == 1 and m_other["pga"] == 1)),
        ("IoU",  f'{m_this["iou"]:.3f}',          m_this["iou"] >= m_other["iou"]),
        ("Sens", f'{m_this["sen"]:.3f}',           m_this["sen"] >= m_other["sen"]),
        ("SaCo", f'{m_this["saco"]:.3f}',          m_this["saco"] >= m_other["saco"]),
    ]
...
def plot_pipeline(
    ...
    examples = _find_examples(
        model, dataset, attn_extractor, gradcam_extractor, grounding_eval, layer
    )
    if not examples:
    ...
    col_titles = ["Input", f"Raw Attention  (layer {layer})", f"Grad-CAM  (layer {layer})"]
    ...
    for row_idx, s in enumerate(examples):
        ...
        overlay_heatmap_annotated(axes[row_idx][1], image_np, s["attn_map"],
                                  boxes, img_size)
        _annotate_metrics(axes[row_idx][1], m_attn, m_grad)
        overlay_heatmap_annotated(axes[row_idx][2], image_np, s["grad_map"],
                                  boxes, img_size)
        _annotate_metrics(axes[row_idx][2], m_grad, m_attn)
